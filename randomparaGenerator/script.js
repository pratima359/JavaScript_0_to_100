...
const paraInput = document.querySelector("#items")
const containerpdata = document.querySelector("#data")


const generate =()=>{
        //to generate random suffled paragraph we will use one suffle function, it will return one suffled array.
 const localText = shuffle(text);

       //Below code for genrate random paragraph
     if (isNaN(paraInput.value) || paraInput.value <0 || paraInput.value >9 ) {
      const createRandomIndex = Math.floor(Math.random()* text.length)
        containerpdata.innerHTML = `<p>${text[createRandomIndex]}</p>`
       
     } else {
      const randomData = localText.slice(0, paraInput.value)
      const paragraph = randomData.map((data)=>{
         return ` <p>${data}</p>`
      })
      containerpdata.innerHTML = paragraph.join('')
